"""再提出まわり
  python asc_submit.py notes <notes.txt> [versionId] … 審査メモを差し替え（versionId 省略時は 1.0 のもの）
...
def notes(path, vid=None):
    """審査メモを差し替える。vid を渡すとそのバージョンの reviewDetail を引いて書く（省略時は 1.0 の RD＝旧挙動）"""
    txt = open(path, encoding="utf-8").read()
    rd = RD
    if vid:
        s, d = a.call("GET", f"/v1/appStoreVersions/{vid}/appStoreReviewDetail?fields[appStoreReviewDetails]=notes")
        if s >= 300 or not d.get("data"): print("reviewDetail not found for", vid, s, d); return
        rd = d["data"]["id"]
    s, d = a.call("PATCH", f"/v1/appStoreReviewDetails/{rd}", {"data": {"type": "appStoreReviewDetails", "id": rd, "attributes": {"notes": txt}}})
    print("notes:", s, "rd=" + rd, "" if s < 300 else d)
